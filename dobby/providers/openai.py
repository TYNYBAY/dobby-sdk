                    ...
                    case "message":
                        ...
                        for content in output.content:
                            if content.type == "output_text":
                                parts.append(TextPart(type="text", text=content.text))
                            else:
            ...
            match event.type:
                ...
                case "response.completed":
                    ...
                    if accumulated_reasoning:
                        parts.append(ReasoningPart(type="reasoning", text=accumulated_reasoning))
                    if accumulated_text:
                        parts.append(TextPart(type="text", text=accumulated_text))
                    # Convert ToolUseEvent (Pydantic) to ToolUsePart (TypedDict) for message history
        ...
        if message["role"] == "assistant":
            ...
            for part in parts:
                match part:
                    ...
                    case {"type": "tool_use", "id": tool_id, "name": name, "inputs": inputs}:
                        tool_calls.append(
                            {
                                "type": "function_call",
                                "call_id": tool_id,
            ...
            parts = message["parts"]
            content_parts: ResponseInputMessageContentListParam = []

            for part in parts:
                match part:
                    case {"type": "text", "text": text}:
                        content_parts.append({"type": "input_text", "text": text})
                    case {"type": "image", "source": source}:
                        if source["type"] == "base64":
                            raise ValueError("Base64 images not supported by `openai.responses.create` api")
                        content_parts.append(
                            {"type": "input_image", "image_url": source["url"]}
                        )
                    case {"type": "document", "source": source, "filename": filename}:
                        match source:
                            case {"type": "url", "url": url}:
                                content_parts.append(
                                    {"type": "input_file", "file_url": url}
                                )
                            case {"type": "base64", "media_type": media_type, "data": data}:
                                content_parts.append(
                                    {
                                        "type": "input_file",
                                        "filename": filename,
                            ...
                            case {"type": "file", "file_id": file_id}:
                                content_parts.append(
            ...
            # Build output parts from various content types
            output_parts = []
            ...
            # If only text parts, join as string; otherwise use array
            if all(isinstance(p, str) for p in output_parts):
                tool_output = "\n".join(output_parts)
                if message.get("is_error"):
                    tool_output = f"Failed to execute tool: \n{tool_output}"
            else:
                # Multimodal output - keep as array
                tool_output = output_parts
            ...
            openai_messages.append(
                {
                    ...
                    "status": "completed" if not message.get("is_error") else "incomplete",
                }
            )

        # adding tool_calls made by llm if present
        if tool_calls:
